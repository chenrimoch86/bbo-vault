...
```mermaid
flowchart TD
    A([Start]) --> B[Describe problem\nin natural language]
    B --> C[Build meta-prompt\nwith prior solution+score pairs]
    C --> D[LLM generates\nnew candidates]
...
flowchart TD
    A([Start]) --> B{Warmstart?}
    B -- Yes\nLLAMBO --> C[Zero-shot candidate sampling\nfrom LLM prior]
    B -- No --> D[Few-shot prompt\nwith observed x, f-x pairs]
    C --> D
    D --> E[LLM predicts\nmean ± std for candidates]
    E --> F[Select next point\nvia EI over LLM output]
...
```mermaid
flowchart TD
    A[Fit GP on observations] --> B{Check GP\nuncertainty σ_mean}
    B -- High σ\nLLM leads --> C[LLM proposes\nnext candidate]
    B -- Medium σ\nLLM filters --> D[BO proposes candidates\nLLM selects best]
    B -- Low σ\nGP confident --> E[Vanilla BO\nmaximize acquisition]
...
```mermaid
flowchart TD
    A([Start]) --> B[LLM generates initial\npopulation of BO algorithm classes]
    B --> C[Evaluate each algorithm\non benchmark functions]
    C --> D[Select top µ algorithms]
    D --> E[LLM crossover:\nmerge two parent algorithms]
    E --> F[LLM mutation:\nmodify one algorithm via prompt]
    F --> G[Add λ offspring\nto population]
    G --> H{Generations\nexhausted?}
    H -- No --> C
...
```mermaid
flowchart TD
    A[Raw training logs\nmetrics, loss curves] --> B[TCS Expert Block\nTrajectory Context Summarizer]
    B --> C[Structured state report\nin natural language]
    C --> D[Small local LLM\nphi4 14B / qwen2.5 32B]
    D --> E[Suggest next\nhyperparameter config]
    E --> F[Train model\nwith suggested config]
    F --> G[Append result\nto training log]
    G --> A
```
